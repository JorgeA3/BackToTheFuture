...
const static char *TAG = "ONE SHOT";
...
#define EXAMPLE_ADC1_CHAN0           ADC_CHANNEL_4  //GPIO36
...
#define EXAMPLE_ADC_ATTEN            ADC_ATTEN_DB_12  //ADC Attenuation factor
...
// Función para calcular el porcentaje de humedad usando la ecuación del modelo de regresión
...
while (1) {
    // Leer solo el canal GPIO36
    ESP_ERROR_CHECK(adc_oneshot_read(adc1_handle, EXAMPLE_ADC1_CHAN0, &adc_raw[0][0]));
    if (do_calibration1_chan0) {
        ESP_ERROR_CHECK(adc_cali_raw_to_voltage(adc1_cali_chan0_handle, adc_raw[0][0], &voltage[0][0]));
        ESP_LOGI(TAG, "GPIO36 Calibrated Voltage: %d mV", voltage[0][0]);
    } else {
        ESP_LOGI(TAG, "GPIO36 Raw ADC Value: %d", adc_raw[0][0]);
    }

    // Calcular y mostrar el porcentaje de humedad
    float humidity = calculate_humidity(voltage[0][0]);
    ESP_LOGI(TAG, "Humedad: %.2f %%", humidity);
    
    vTaskDelay(pdMS_TO_TICKS(1000)); // espera 1 s antes de la siguiente lectura
}
